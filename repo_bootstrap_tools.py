...
def reshape_json(raw, schema, host="http://127.0.0.1:8080"):
    """Repair stage: take prose a THINKING call already produced and force it
    into `schema`. Does not re-reason -- it only reformats, so the judgement
    from the first call survives while the format becomes guaranteed."""
    if not (raw or "").strip():
        return {}
    try:
        out = llm_call(
            system=("You convert text into JSON matching a schema. Extract "
                    "what is present; do not invent, do not reason further."),
            prompt="Text:\n%s\n\nReturn it as JSON matching the schema." % raw[:6000],
            max_tokens=1200, strict_schema=schema, host=host)
        import json as _j
        return _j.loads(out) if out.strip() else {}
    except Exception:
        return {}
...
def llm_call(prompt, system="You are a helpful assistant. Answer concisely.",
             ...
             format_json=False, num_ctx=131072, strict_schema=None):
    ...
    if strict_schema:
        # Schema-enforced decoding. Requires thinking OFF: with the think block
        # enabled the grammar is applied to a stream the model is still using
        # to reason, and content comes back empty. Costs judgement -- use only
        # where the output is a lookup or a reformat, never where the model
...
def make_bootstrap_handlers(repo_dir, base_env_vars=None, fail_to_pass=None,
    ...
    state["pass_to_pass"] = list(pass_to_pass or [])
    state["repo_name"] = repo or ""
...
BOOTSTRAP_SYSTEM_PROMPT = (
    ...
    "developer.\n\n"
    "ENVIRONMENT KNOWLEDGE BASE - consult these accumulated rules before "
    "guessing; they override defaults (full notes in ENV_KNOWLEDGE.md):\n"
    "  - Wrong Python version is the #1 cause of env failure. Get it right "
    "first; building/importing does NOT prove the env is correct.\n"
    "  - Backend by Python: uv provides 3.8+ only and CANNOT build 3.6/3.7 - "
    "use conda (micromamba) to mint those interpreters, then pip for packages.\n"
    "  - Prefer pip + wheels (they reach cp36). Do NOT let >= bounds float to "
    "the newest (numpy 2.x / pandas 2.x break old code); pin to the declared "
    "floor, bumped to the oldest version with a wheel for THIS Python.\n"
    "  - Optional test deps (pandas, matplotlib) are NOT pulled by installing "
    "the library; install them explicitly or the tests silently skip.\n"
    "  - Build from source (--no-build-isolation, cython+setuptools present) "
    "when no wheel exists; fall back to conda-forge when a version is yanked or "
    "removed from PyPI or its sdist no longer builds.\n"
    "  - Wrong-floor errors lie: a dependency deprecation can be fatal in "
    "warnings-as-errors repos (astropy), and a missing system C lib "
    "(portaudio.h) masquerades as a Python error - look one layer down.\n\n"
    "RECON (do this first):\n"
    ...
    "  9b. To verify: call run_smoke_test WITH NO ARGUMENTS — the harness "
    "runs a standard known-good test for you. Do NOT guess test ids; do NOT "
    "use run_sanity to hunt for tests.\n"
