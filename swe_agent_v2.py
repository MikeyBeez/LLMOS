            ...
            if state["widen_turns"] > _max:
                ...
                if isinstance(handlers, dict):
                    ...
                    handlers["_widen_notice"] = (
                        ...
                        "it implements the rule. When you are unsure how far to go, "
                        "use the cost of being wrong: be GENEROUS about what you "
                        "ACCEPT (inputs, types, guards -- accepting a sibling type "
                        "you will not get costs nothing, rejecting one breaks a "
                        "caller silently), and STINGY about what you PRODUCE "
                        "(output, formatting, behaviour -- add nothing the rule "
                        "does not require, since extra output is exactly what a "
                        "test pins down). STEP 3 -- call widen_check(rule=<your "
                        "sentence>, variants=[2-4 short scripts]). At least one "
                        "variant must be ADVERSARIAL: a case you suspect your fix "
                        "does NOT handle -- a sibling type (list->tuple, str->bool), "
                        "the empty/None case, or the reverse direction of a "
                        "read/write pair. A variant that only re-confirms the "
                        "example teaches nothing. Each exits 0 only if the fix "
                        "holds for that case. All green completes verification; any "
                        "red means the rule is not yet implemented. (%d/%d)"
