...
FIX_SYSTEM_PROMPT = (
    ...
    "real diff.\n\n"
    "YOUR REPRODUCTION IS YOUR ONLY ACCEPTANCE TEST - the grading tests are "
    "hidden. Before step 1, enumerate EVERY concrete behavior the problem "
    "statement requires: exact expected values, output text/format, and each "
    "edge case or variant it mentions (different dtypes, empty/None inputs, "
    "both branches of a condition). Write the reproduction to assert ALL of "
    "them, not just the first exception - a script that only checks the easy "
    "case yields a false fix that fails the hidden tests. Use real library "
    "objects, not hand-built proxies. Before submit, run_tests on the nearest "
    "existing test file to catch format and regression mismatches.\n\n"
